...
class FaceSwap(object):
    def __init__(self, predictor_path):
        ...
        self.predictor = dlib.shape_predictor(self._predictor_path)
    ...
    def get_landmarks(self, im):
        ...
        # TODO: If too many faces, pick a random one for the swap?
        if len(rects) > 1:
            raise self.TooManyFaces
        # TODO: If no faces, try to insert the face somewhere in the bg?
        if len(rects) == 0:
            raise self.NoFaces
    ...
    def do_swap(self, head_image, face_image, output='/tmp/output.jpg'):
        im1, landmarks1 = self.read_im_and_landmarks(head_image)
        im2, landmarks2 = self.read_im_and_landmarks(face_image)
